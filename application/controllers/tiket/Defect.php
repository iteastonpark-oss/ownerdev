...
class Defect extends CI_Controller
{
	...
	public function view($tabs = 'open')
	{
		...
		$data['judul'] = "Data Defect " . strtoupper($tabs);
		$data['tabs'] = $tabs;
		$data['jatuh_tempo'] = isset($_POST['jatuh_tempo']) ? $_POST['jatuh_tempo'] : 0;

		$data['tombol_view'] = "";
		if ($tabs == 'open') {
			$data['tombol_view'] .= $this->apl->anchor('<a href="' . site_url('tiket/defect/add') . '" 
...
<i class="fa fa-file-excel"></i> Export</a>';
	...
	public function add()
	{
		$data['judul'] = "Tambah Data Defect";
		$data['page'] = 'tiket/form'; //Halaman di tampilkan
		$data['submit'] = 'tambah'; //Halaman di tampilkan
		$data['no_form'] = $this->apl->counter_view("DEFECT/EPR-TR") . "/" . $this->apl->counter_code("DEFECT/EPR-TR");
		$data['pekerjaan'] = $this->apl->getSelectedData("db_pekerjaan", array('hapus' => 0));
		$this->load->view('home', $data);
	...
	public function actions()
	{
		...
		if ($submit == 'tambah') {
			...
			$this->apl->counter("DEFECT/EPR-TR");
