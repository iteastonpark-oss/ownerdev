...
class Lainnya extends CI_Controller
{
	...
	public function index()
	{
		...
		$tag_default=$this->apl->get_nilai_pilih("db_tag","id_tag",array('tipe'=>7));
		$data['id_tag'] = (isset($_POST['id_tag'])) ? $_POST['id_tag'] : $tag_default;
		...
		/*
		$data['tombol_view'] = '<div class="btn-group pull-right">'
			. $this->apl->anchor(anchor(
				'#modal_form',
				'<i class="fa fa-plus"></i> Add Payment',
				'data-id="' . $data['id_unit'] . '" data-toggle="modal" 
				class="bayar btn btn-neutral"'
			), "tambah " . $this->modul, $this->modul);
			*/
		$data['tombol_view'] = '<div class="btn-group pull-right">';
		$data['tombol_view'] .= $this->apl->anchor('<div class="dropdown">
			<button class="btn btn-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false">
			<i class="fa fa-plus"></i> Add Payment
			</button>
			<div class="dropdown-menu">
			  <a class="bayar dropdown-item" data-tipe="1" data-toggle="modal"  href="#modal_form">Unit</a>
			  <a class="bayar dropdown-item" data-tipe="2" data-toggle="modal"  href="#modal_form">Vendor</a>
			  <a class="bayar dropdown-item" data-tipe="3" data-toggle="modal"  href="#modal_form">Lainnya</a>
			</div>
		  </div>', "tambah " . $this->modul, $this->modul);

		$data['tombol_view'] .= '<a href="' . site_url('bayar/lainnya/export_csv/' . $data['id_tag'] . '?id_unit=' . $data['id_unit']
			. '&tanggal_awal=' . $data['tanggal_awal'] . '&tanggal_ahir=' . $data['tanggal_ahir']) . '" 
class="btn btn-info">
<i class="fa fa-file-excel"></i> Export</a></div>';

		$data['field'] = $this->bayar->getDataBayarLainnya($data['id_unit'], $data['id_tag'], $data['tanggal_awal'], $data['tanggal_ahir'])->get()->list_fields();
		$data['jumlah'] = $this->bayar->getDataBayarLainnyaJumlah($data['id_unit'], $data['id_tag'], $data['tanggal_awal'], $data['tanggal_ahir']);
		$data['page'] = 'bayar/lainnya/view'; //Halaman di tampilkan
	...
	{
		$result = $this->bayar->getDataBayarLainnya($_GET['id_unit'], $id_tag, $_GET['tanggal_awal'], $_GET['tanggal_ahir'])->get();
	...
	public function ajax_list()
	{
		...
		$column_search = array(
			'(CASE WHEN bayar.tipe = 1 THEN bast.kode
	  ...
	  END)',
		...
		$list = $this->crud_model->get_data(
			$this->bayar->getDataBayarLainnya($id_unit, $id_tag, $tanggal_awal, $tanggal_ahir),
			$column_search,
			$column_order
		);
		...
		foreach ($list->result_array() as $data) {
			...
			$aksi = '<div class="dropdown">'
				...
				. anchor(
					site_url('bayar/lainnya/cetak?id=' . $r[$jumlah - 1]),
	...
	public function form_bayar($tipe = '1')
	{
		$data['submit'] = 'tambah';
		$data['tipe'] = $tipe;
		$this->load->view('bayar/lainnya/payment', $data);
	}
	...
	public function cetak()
	{
		...
		$data['l'] = $this->apl->getSelectedData(
			"bayar_lainnya",
			array('id_bayar' => $id, 'hapus' => 0)
		)->row();
		...
		$this->load->view('bayar/lainnya/cetak', $data);
	}
