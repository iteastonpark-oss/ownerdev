...
<div class="card">
			...
			<div class="row">
				<div class="col-md-6">
					<div class="form-group">
						<label for="" class="control-label">Pilih Unit BAST</label>
						<?= $this->dropdown_model->getDropdownUnitBast('id_bast', $id_bast,
							'class="form-control satu" onchange="this.form.submit()"'); ?>
					</div>
				</div>
				...
				<div class="col-md-3">
					<div class="form-group">
						<label for="" class="control-label">Tahun Tagihan</label>
						<?php
	...
	if ($id_bast != '') {
		?>
		<form action="<?= site_url($controller . '/add_act'); ?>"
			  method="post">
			<div class="card-body">
				<div class="row">
					...
					<table class="table table-sm table-borderless small table-striped ">
						...
						<thead>
						<th colspan="10"><span class="heading-small text-muted row">Tagihan Periode (SF & SC)</span>
						</th>
						<tr class="bg-neutral">
							<th>Check</th>
            ...
            class="checkbox" name="tanggal_tagihan[]" value="' . date('Y-m-' . $tanggal_periode, $month) . '"></th>';
					...
					<table class="table table-sm table-borderless small table-striped mt-2 ">
						<thead>
						<tr class="info">
							<th colspan="10"><span class="heading-small text-muted row">Tagihan
										Administrasi</span></th>
						</tr>
						<tr class="bg-neutral">
							<th>Check</th>
							<th style="width:10%">BULAN</th>
            ...
            class="checkbox" name="tanggal_tagihan_adm[]" value="' . date('Y-m-' . $tanggal_periode, $month) . '"></th>';
					...
					<table class="table table-sm table-borderless small table-striped">
						...
						foreach ($list_adm_invoice->result() as $data_tagihan) {
							$checked = ($b->kirim == '2' && $data_tagihan->id_tag == 9) ? '' : 'checked';
							echo $checked;
							?>
							<tr>
								<td>
									<input type="checkbox"
										   ...
										   <?= $checked; ?>>
									<input type="hidden"
										   name="nama_tagihan_invoice[<?php echo $data_tagihan->id_tag; ?>]"
						...
						<table class="table table-sm table-borderless small table-striped">
							...
							foreach ($list_utility->result() as $data_utility) {
								...
								<tr>
									<td>
										<input type="checkbox"
											   class="checkbox" name="utility[]"
											   value="<?php echo $data_utility->id; ?>"
											   checked="checked">
			...
			<div class="card-footer">
				<div class="btn-group pull-right">
					...
					<button class="btn btn-primary" name="print"><i class="fa fa-print"></i> Simpan & Print
					</button>
				</div>
			</div>
		</form>
